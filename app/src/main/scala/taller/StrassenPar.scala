...
object StrassenPar {
  ...
  def strassenMultiply(a: Array[Array[Int]], b: Array[Array[Int]]): Array[Array[Int]] = {
    ...
    if (n == 1) {
      ...
      val results = tasks.par.map(_.apply()).toArray
  ...
  def splitMatrix(matrix: Array[Array[Int]]): (Array[Array[Int]], Array[Array[Int]], Array[Array[Int]], Array[Array[Int]]) = {
    val n = matrix.length
    val newSize = n / 2
    val a11 = Array.ofDim[Int](newSize, newSize)
    val a12 = Array.ofDim[Int](newSize, newSize)
    val a21 = Array.ofDim[Int](newSize, newSize)
    val a22 = Array.ofDim[Int](newSize, newSize)

    for (i <- 0 until newSize; j <- 0 until newSize) {
      a11(i)(j) = matrix(i)(j)
      a12(i)(j) = matrix(i)(j + newSize)
      a21(i)(j) = matrix(i + newSize)(j)
      a22(i)(j) = matrix(i + newSize)(j + newSize)
    }
